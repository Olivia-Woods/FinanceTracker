document.addEventListener("DOMContentLoaded", () => {
  ...
  const transactionTableBody = document
  ...
  // Load Transactions: localStorage
  let transactions = JSON.parse(localStorage.getItem("transactions")) || [];

  // Save Transactions: localStorage
  ...
  const displayTransactions = (filteredTransactions = transactions) => {
    transactionTableBody.innerHTML = filteredTransactions
      .map(
        (transaction, index) => `
            <tr>
  ...
  // Convert Amount from AUD to USD
  ...
  const filterTransactions = () => {
    ...
    const filteredTransactions = transactions.filter((transaction) => {
      ...
      const matchesDate =
        (!filterStartDate.value || rowDate >= startDate) &&
